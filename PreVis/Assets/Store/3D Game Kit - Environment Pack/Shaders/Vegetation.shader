Shader "Custom/Vegetation" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			//float rValue = _EmissiveColor.x + (2 + sin(_Time * 1 + (IN.worldPos.x)));

			//_EmissiveColor = half4(rValue, _EmissiveColor.y, _EmissiveColor.z, 1.0f);

			o.Emission = tex2D (_EmissionMap, IN.uv_MainTex) * _EmissiveColor * (2 + sin(_Time * .5f + (IN.worldPos.z) + sin(_Time * 1 + (IN.worldPos.x))));
			//o.Alpha = c.a;
		}
		ENDCG
	}
